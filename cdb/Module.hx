	...
	public static function build( file : String, ?typeName : String ) {
		...
		for( s in data.sheets ) {
			...
			for( c in s.columns ) {
				...
				switch( c.type ) {
				case TInt, TFloat, TString, TBool, TImage, TColor, TFile, TTilePos, TDynamic, TGradient, TCurve:
					var cname = c.name;
					fields.push({
						name : "get_"+c.name,
						pos : pos,
			...
			if( polyFields != null ) {
				...
				for( col in s.columns ) {
					...
					var colValue = macro obj.$colName;
					switch( col.type ) {
					case TRef(s):
						var fname = fieldName(s);
						colValue = macro $i{modName}.$fname.resolve($colValue);
					default:
					}
